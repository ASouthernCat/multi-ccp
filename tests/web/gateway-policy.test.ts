...
describe("gateway Web UI policy", () => {
  ...
  it("keeps protected upstream deletion clickable and reserves wait cursors for busy actions", async () => {
    ...
    expect(app).not.toContain('openCcrPanel');
    expect(app).not.toContain('/api/ccr/');
    expect(html).not.toContain('ccrDialog');
    expect(html).not.toContain('data-preset-filter="ccr"');
    expect(server).not.toContain('/api/ccr/');
    expect(server).not.toContain('getCcrStatus');
